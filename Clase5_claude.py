...
if __name__ == "__main__":
    # Create the YaEmpanadas system
    sistema = YaEmpanadas()
    
    # Create a new order
    num_pedido = sistema.crearORecuperarPedido("123-456-7890", "Calle Falsa 123")
    print(f"Nuevo pedido creado con número: {num_pedido}")
    
    # Add some empanadas to the order
    sistema.cargarEmpanadas(num_pedido, Sabor.CARNE, 3)
    sistema.cargarEmpanadas(num_pedido, Sabor.POLLO, 2)
    sistema.cargarEmpanadas(num_pedido, Sabor.JAMON_Y_QUESO, 4)
    
    # Add more of the same type
    sistema.cargarEmpanadas(num_pedido, Sabor.CARNE, 2)
    
    # List the complete order
    print("\nDetalles del pedido:")
    print(sistema.listarPedidoCompleto(num_pedido))
    
    # Create another order with the same contact info (should return existing order)
    num_pedido2 = sistema.crearORecuperarPedido("123-456-7890", "Calle Falsa 123")
    print(f"\nPedido recuperado con número: {num_pedido2}")
    
    # Create a new order with different contact info
    num_pedido3 = sistema.crearORecuperarPedido("987-654-3210", "Avenida Siempreviva 742")
    print(f"\nNuevo pedido creado con número: {num_pedido3}")
    sistema.cargarEmpanadas(num_pedido3, Sabor.VERDURA, 2)
    sistema.cargarEmpanadas(num_pedido3, Sabor.HUMITA, 3)
    
    # List all orders
    print("\nLista de todos los pedidos:")
    for i in range(1, sistema.next_num):
        print(sistema.listarPedidoCompleto(i))
        print("-------------------")
